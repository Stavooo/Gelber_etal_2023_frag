...
prepare_data_heatmap_disp_samp <- function(path, var1, var2) {
  ...
  data_geo_1 <- data_geo_1 %>%
    select(-n_patches, -n_samples, -individuals, -shannon, -patch_individuals_mean, -patch_species_mean, -patch_shannon_mean, -sample_individuals_mean, -present_species, -sample_shannon_mean) %>%
    rename(richness_sam = sample_species_mean)
  ...
  data_lm_geo_sam <- data_geo_1 %>%
    group_by((!!as.symbol(var1)), (!!as.symbol(var2))) %>%
    nest() %>%
    mutate(lm = map(data, ~ lm(richness_sam ~ fragmentation, data = .x) %>%
      tidy())) %>%
    unnest(lm) %>%
    filter(term == "fragmentation") %>%
    select(-term)

  data_lm_gedo_sam <- data_gedo_1 %>%
    group_by((!!as.symbol(var1)), (!!as.symbol(var2))) %>%
    nest() %>%
    mutate(lm = map(data, ~ lm(richness_sam ~ fragmentation, data = .x) %>%
      tidy())) %>%
  ...
  results <- list(geo_lm_sam = data_lm_geo_sam, gedo_lm_sam = data_lm_gedo_sam, data = data_1, steps = steps)
  return(results)
}
